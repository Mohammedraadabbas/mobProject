...
function showChampionships(championship) {
  ...
  let countdown = setInterval(function () {
    let deadline = new Date("Sep 17, 2022 16:00:00").getTime()
    ...
    console.log(Math.floor((defBtDLandST / (1000 * 60 * 60 * 24))))
    ...
    if (defBtDLandST <= 0) {
      ...
      let date = document.querySelector(`#${data.id} .deadline-time`)
      gsap.to(date, { display: "none", opacity: 0, duration: 5, ease: "expo" })
